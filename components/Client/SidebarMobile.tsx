  ...
  return (
    <div className="fixed top-0 z-30 h-screen w-full overflow-x-hidden">
      <AnimatePresence>
        <motion.div
          variants={{
            initial: { x: "100%" }, // Cuando se inicia el sidebar, se mueve desde la derecha
            exit: { x: "100%" },    // Cuando se cierra, se mueve hacia la derecha
            animate: { x: "0%" },   // Cuando se abre, se mueve hacia la izquierda
          }}
          initial="initial"
          animate="animate"
          transition={{
        ...
        >
          ...
          <StayConnected className="mt-4800svh gap-y-6 text-sm leading-none text-[#2b3530]/80 [&_div]:gap-x-8 [&_svg]:h-2400svh [&_svg]:w-auto" />
        </motion.div>
      </AnimatePresence>
    </div>
  );
